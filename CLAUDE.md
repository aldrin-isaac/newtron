...
- `1node-vs-config`: VALIDATED — 25/25 PASS (loopback `--no-deploy`, ~2s); re-run **2026-07-01 against main @ post-#347** (per-file zones #346 + the `node`/`zone` CLI `-x` fix #347 landed) as ron under the enforced + `--audit`-enabled server — confirms per-file zones (`zones/<zone>.json`) cause no regression on the CLI-config path; prior re-run 2026-06-30 against main @ post-#344 (the per-network audit arc #340–#343 landed); original 2026-06-20 against main @ 7560efd. Re-run green at every step of the register-network → IPVPN/VRF-collapse → global-platforms arc (#245, #251, #249, #250, #252, #253, #256, #257, #258, #259). The suite grew from the original 13 to 25 scenarios; this is the current full set. Loopback mode has no cold-deploy/ASIC-convergence exposure (no real switch), so it's the fast regression gate for spec/wire-shape changes. **Session caveat (the exact mechanism):** the suite uses the `newtron-cli` action, which newt-server execs **server-side** — inheriting newt-server's env (so `NEWTRON_USER` is unset unless the server was launched with it) and passing `--server http://127.0.0.1:18080` but **not** `--user`. So it can't disambiguate when MULTIPLE sessions are cached and bails with "multiple cached sessions" (symptom: `setup-device` FAILs → 14 skips, ~9/2/14 — looks like a regression but isn't). This bites right after running `1node-vs-auth-deployed`, which caches ron **and** mallory. Fix: clear `~/.newtron/sessions/` down to the single `ron@127.0.0.1_18080.json` (matching the exec's `--server` host), or launch newt-server with `NEWTRON_USER=ron` in its env. **Second mechanism, same 9/2/14 symptom:** the server-side exec resolves the `newtron` binary from the newt-server process's `$PATH` (steps_cli.go). If newt-server was launched (e.g. via `nohup bin/newt-server …`) with a `$PATH` that lacks the repo `bin/`, every `newtron-cli`-action step fails with `exec: "newtron": executable file not found in $PATH` — the 9 `action: newtron` (in-process/HTTP) scenarios still pass, the 2 `action: newtron-cli` scenarios (setup-device, topology-show) fail, and their 14 dependents skip → the same 9/2/14. Fix: put the freshly-built binary on the server's PATH (`ln -sf "$PWD/bin/newtron" ~/go/bin/newtron` — `~/go/bin` is already on PATH; the exec resolves fresh per call, no restart needed). Diagnose the split with `--junit out.xml`, which carries the per-step failure message the streaming summary omits.
