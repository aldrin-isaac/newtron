...
When encountering a SONiC-specific issue (config reload, frrcfgd, orchagent, VPP), check `docs/rca/` first — there are 46 documented pitfalls with root causes and solutions.
...
after the **service** (`Vrf_<SERVICE>` shared / `Vrf_<SERVICE>_<E|P|V##>` interface,
single-letter interface), not the ipvpn; `vrf bind-ipvpn <vrf-name> <ipvpn>` takes
the VRF explicitly (ipvpn-derived naming deleted); `vrf_type` immutable. IFNAMSIZ
enforced (RCA-052): service-name caps 5 interface / 11 shared, apply-time
fail-close, schema `MaxLength`/`MaxLengthWhen`. New **intent-snapshot** feature
(`intent snapshot` / `snapshot-diff`, `GET .../intent/snapshot`) catches
residual/orphaned intent (NEWTRON_INTENT is drift-excluded). Demo services renamed
to the compact convention `[E]`+`IRB|BRD|RTD`+`[1|2]` (EIRB1/EIRB2/EIRB/EBRD/BRD/
IRB/RTD/ERTD1). **Cold, both platforms, all green — 2node-vs-primitive 25/25,
2node-ngdp-primitive 22/22, 2node-vs-service 7/7, 2node-ngdp-service 6/6**;
`go test ./...` + vet green. The service-suite cold runs earned their keep,
catching three bugs the primitives (different provisioning path) could not: (1) the
standalone `bind-ipvpn` topology step lacked `vrf_name` (the primitives bind at the
scenario level, already fixed); (2) the manual-IRB pre-provisioning named its VRF
`Vrf_IRB` while the shared evpn-irb service now derives `Vrf_EIRB1/2` — on main both
derived `Vrf_IRB` so they shared one VRF, and the per-service split made removing
the service reap the L3VNI transit the surviving manual bind still held → drift →
deprovision blocked (fixed: manual pre-provisioning uses the per-service VRF name);
(3) a vacuous verify-clean L3VNI-map check keyed by VRF (`VNI50400_Vrf_EIRB1`) not
by the L3VNI VLAN (`VNI50400_Vlan3998`) — corrected to the real key and re-run
non-vacuous on both platforms. **Process note:** a cold provision `config reload`
hit a one-off `orchagent SAI_API_HOSTIF` create-storm; it did **not** reproduce on
re-run and the identical reload succeeded warm (`EXIT=0`) — a transient cold-boot
race, NOT host degradation (host measured load ~2/24, 85 GB free, iowait/steal 0;
the reflex "degraded host" claim was retracted for lack of basis). Root-cause
discipline mattered twice: the config-reload-wipe and degraded-host hypotheses were
both wrong and corrected by the run timeline + host metrics before any fix landed.
...
**Full-sweep 2026-09-26 (post-#497)** (referenced by the per-suite records below as "the
2026-09-26 sweep"): all 13 suites sequentially on main @ post-#497,
...
bring-up; `NEWTRON_USER=ron`; identities cached via the two login scripts;
each lab destroyed as soon as its suites finished). **190 scenarios — 0
failures, 1 by-design skip, ~2h6m total.** Zero fixes were needed anywhere —
the first all-suite sweep in the project's history to pass every suite on
the first attempt. Five suites got their first ledger record in this sweep
(1node-vs-basic, 2node-vs-drift, 2node-vs-drift-actuated, simple-vrf-host,
2node-ngdp-service).
...
- `1node-vs-config`: VALIDATED — 28/28 PASS in the 2026-09-26 sweep (loopback, 2s). The suite is 28 scenarios now (the 25 below predates its growth). Prior: 25/25 PASS in the 2026-07-07 sweep (loopback, 2s). Prior: 25/25 (loopback `--no-deploy`, ~2s); re-run **2026-07-02 against main @ post-#366** (the DRY sweep #356–#366) as ron with 12 identities cached — 25/25, confirming the auth/client/URL-resolution changes cause no regression on the CLI-config path; prior re-run **2026-07-01 against main @ post-#347** (per-file zones #346 + the `node`/`zone` CLI `-x` fix #347 landed) as ron under the enforced + `--audit`-enabled server — confirms per-file zones (`zones/<zone>.json`) cause no regression; original 2026-06-20 against main @ 7560efd. Re-run green at every step of the register-network → IPVPN/VRF-collapse → global-platforms arc (#245, #251, #249, #250, #252, #253, #256, #257, #258, #259). The suite grew from the original 13 to 25 scenarios; this is the current full set. Loopback mode has no cold-deploy/ASIC-convergence exposure (no real switch), so it's the fast regression gate for spec/wire-shape changes. **Running it under enforcement (two operational notes):** (1) *Operator identity.* Submit the run with `NEWTRON_USER=<user>` (e.g. ron) to select the operator among cached sessions. The old hard "multiple cached sessions" CLI bail is gone — since #357 an ambiguous cache resolves to an *empty* Bearer, not an error — but under `--enforce-authorization` an empty Bearer 401s, so an explicit `NEWTRON_USER` is still how you pick the operator when more than one session is cached. The suite's own `newtron-cli` action needs no session cache at all: the runner forwards the run's operator (or a scenario's `as:`) Bearer to the server-side exec via `NEWTRON_BEARER` (`Runner.scenarioBearer`, steps_cli.go). (2) *Binary on PATH.* That exec resolves the `newtron` binary from newt-server's `$PATH`; symlink the fresh build on (`ln -sf "$PWD/bin/newtron" ~/go/bin/newtron` — `~/go/bin` is already on PATH, resolved fresh per call, no restart needed) or the 2 `newtron-cli` scenarios (setup-device, topology-show) fail with `exec: "newtron": … not found` while the 9 HTTP scenarios pass and 14 dependents skip → a 9/2/14 split. Failing steps stream their name + message in the live summary since #417; `--junit out.xml` still carries the full untruncated detail.
- `1node-vs-architecture`: VALIDATED — 32/32 PASS in the 2026-09-26 sweep (47m37s, on the cold-deployed 1node-vs lab). The long pole of any sweep — budget ~48m for it alone. Prior: 32/32 PASS in the 2026-07-07 sweep (49m0s, on the then-running 1node-vs lab). Prior: 32/32 (re-run 2026-05-31 against main @ ec16631 with bin/newt-server on :18080; 47m33s on the deployed 1node-vs lab)
- **#486 rung 0a (`spec-diff`) was REMOVED 2026-09-25.** The ledger entries below reference an
  `intent spec-diff` read and on-device suite checks that no longer exist. It was built from an
  external doc-reviewer's architectural suggestion with no operator demand, and on review the
  accounting did not hold: it added a public API route, a CLI command, two public types, a client
  method and ~150 lines to surface *one* symptom, while the bug **class** behind that symptom was
  actually found by extending `TestOpRoundTrip` with a filter + QoS-bearing service — ~30 lines,
  zero new surface. **The fixture extension was kept; the feature was deleted** (greenfield: delete,
  don't deprecate), consistent with this project's standing precedent of deferring speculative
  enhancements until a concrete need surfaces (resolution-provenance, saga). The bug fixes the
  episode produced (#492 ACL filter round-trip, #494 projection partial-modify merge + the §25
  acl/qos convergence) are unaffected and remain. See `docs/newtron/spec-diff-design.md` for the
  full justification and the decision record.
- `2node-vs-primitive`: VALIDATED — 25/25 PASS **cold-deploy** in the 2026-09-26 sweep (9m39s). Prior: 25/25 PASS **cold-deploy** 2026-09-24 (9m33s, fix/projection-partial-modify-merge @ #494). Cold validation of the **§25 two-writer convergence** — the root fix the two entries below only worked around. `acl|<name>` and `interface|<name>|qos` each had two writers *and* two deleters (the standalone ops vs the ApplyService/RemoveService composite, hand-writing the same records); they diverged on `filter` (#492) and the projection-level twin of the class was #494, both fixed by aligning a copy. Now there is one owner per record — `createACLIntent`/`deleteACLIntent` (acl_ops.go) and `createQoSBindingIntent`/`deleteQoSBindingIntent` (qos_ops.go), forward and reverse named and shipped together (§15), with `aclKey`/`aclRuleKey`/`qosBindingKey` following the existing `bindingKey` idiom. Every caller routes through them; the only `writeIntent`/`deleteIntent` for these records now live inside the owners. **The direction matters and the obvious one was wrong:** `op()`'s own doc directs composites like ApplyService to manage their own ChangeSet, so routing ApplyService *through CreateACL* would have forced a composite into the simple-CRUD helper and required a caller-specific escape hatch — the owner is the intent **record**, not the operation. Deliberately left un-converged because they are decisions, not copies: each caller's ChangeSet lifecycle, and its idempotency behaviour (CreateACL returns an empty set when the record exists; ApplyService merges the new port into the shared ACL). The irb path still records no bind-qos intent (unreplayable — would abort the next rebuild), expressed by *not calling* the owner, never a flag on it. Covers both directions cold: acl-lifecycle + qos-lifecycle (standalone owners), dataplane-update-acl-rule (`aclRuleKey`), service-on-portchannel, service-lifecycle (service-derived ACL + spec-diff), teardown-* + verify-clean (reverse owners). Paired with 2node-vs-service 7/7 for the deprovision path. Prior: 25/25 PASS **cold-deploy** 2026-09-22 (9m25s, fix/projection-partial-modify-merge @ #494). Cold validation of the **projection partial-modify fix** — the class behind the #492 ACL-filter symptom. Extending `TestOpRoundTrip` to apply a **filter + QoS-bearing service** (it only ever applied a bare `routed` service) caught it: `render()`'s add/modify case commented *"HSET merge"* but the code **full-replaced** the typed projection struct via the hydrator, so any **partial `cs.Update`** to a full-replace (typed) hydrator table dropped every field it omitted — `updateAclPorts` writing `{ports}` wiped `type/stage/policy_desc` off a service ACL in the projection, diverging from the device (where the real HSET merges, §48) and would show false drift after any rebuild. Class = partial `cs.Update` to a typed hydrator table: **ACL_TABLE** (`updateAclPorts`, `bindAclConfig`) + **BGP_PEER_GROUP_AF**; `mergeHydrator` tables (DEVICE_METADATA, …) already merged (the design's mechanism for partial-update tables) — it is **not** "all `Create*`". Fixed once at the projection layer: `render` merges a `ChangeModify` over the row's existing fields before hydrating, mirroring the device HSET. Delivery untouched (`cs.Apply` still HSETs only changed fields; teardown-replace `ChangeReplace` still full-replaces → §48 split preserved); rebuild budget preserved (merge source is the existing `prior` snapshot, taken during reconstruction only when a modify is present via `cs.hasModify` — 88ms @ 1000 intents, unchanged). This **subsumes** the #492 two-writer note below: the ACL/QoS service-object paths now round-trip losslessly and the extended fixture guards the class. Cold clean incl. dataplane-update-acl-rule, service-on-portchannel, service-lifecycle (spec-diff steps), verify-clean; `bridged` clean (host healthy); `go test ./...` (25 pkgs) + vet green. Prior: 25/25 PASS **cold-deploy** 2026-09-22 (8m56s, main @ post-#492, first cold run with the `spec-diff` on-device check in service-lifecycle, #486 rung 0a). **The cold run earned its keep — it found a bug the unit tests structurally could not.** The added `spec-diff-up-to-date` check asserts `intent spec-diff` returns empty on a freshly-provisioned device (device applied intent == what current specs would apply). The FIRST run FAILED it: spec-diff reported a spurious `filter: FILTER_IRB → ""` on the service-created ACL. Root cause (fixed #492): the `acl|<name>` intent has **two writers** — apply-service hand-writes it with the `filter` provenance (§24/§25), while the create-acl *replay* goes through `CreateACL`, which dropped `filter`. `TestOpRoundTrip` missed it (fixture uses standalone ACLs, no filter — a coverage gap). Fixed by recording+replaying `filter` through `CreateACL`; regression test verified to fail without the fix. On-device detection also proven: a real `network.json` edit → watcher reload → spec-diff reported the moved params (`interface|Vlan400|service` l2vni 50400→50888, `macvpn|400` vni), restored to empty on revert. **Note (closed 2026-09-24):** the two-writer §25 duplication this fix only *aligned* is now removed — one owner per intent record, every caller routed through it (see the 09-24 entry above). Two corrections to what this note originally claimed: the root fix is **not** "converge apply-service onto CreateACL" (`op()` directs composites to own their ChangeSet, so the owner is the intent *record*, not the operation), and the duplication was never just apply-service-vs-CreateACL — it was four twins, create **and** delete, across ACL **and** QoS (§15 makes the reverse half mandatory, not optional). Prior: 25/25 PASS **cold-deploy** 2026-07-31 (9m41s, main @ post-#470, first cold run since the 07-19 VRF sweep and the first with the **spec watcher on by default** (#468/#470). Twelve PRs had merged unvalidated, three of them runtime-affecting: typed 400s for required-parameter and topology-link validation (#465), newtlab 404-not-500 for an absent lab/node (#463), and the default-on watcher. The watcher was the one needing device-level proof — newtrun provisions via topology-reconcile, which writes specs continuously while SSH sessions are live, and a spurious reload drains node actors. Clean: no reload churn, no dropped sessions. **Process note:** getting to the start line surfaced a teardown bug (fixed #471) — `newtlab destroy` reported success while leaving 3 VMs running and holding ports, because the CLI builds a bare `&Lab{NetworkID:…}` with no StateDir and the sweep scoped to `""`. Verify `ps -eo comm | grep qemu` is empty after a destroy before trusting a "cold" run; a redeploy over orphans collides on SSH ports. Prior: 25/25 PASS **cold-deploy** 2026-07-17 (9m34s, feat/status-kind-aware-legs — the A4 kind-aware `/status` LAG/SVI legs + the portchannel member-storage fix, PR #441). Getting to green was an RCA-047 masterclass in *not* blaming the diff. The `bridged` ASIC poll (`poll-asic-vlanmember-ready ≥ 2`) flaked hard — orchagent stalling at **1 of 3** SAI bridge ports, idle (0% CPU, no retry storm, no SAI error), with CONFIG_DB fully correct — at a **high** rate on a host degraded by this session's many cold deploy/destroy cycles (there every branch *and* main failed), which a **host reboot** dropped to intermittent. A degraded-host `main` run failing briefly implicated a pre-existing regression; a full **bisect** corrected that: cold-green at 4c10537 (#439) with a **byte-identical emitted CONFIG_DB** (VLAN/VLAN_MEMBER/PORT) to HEAD, cold-green at 796dbb4 (origin/main) and at 15f439d (portchannel commit), while HEAD flaked once then passed the full 25/25 on rerun. Proof the A4 read-surface can't reach the bridged write path: `VLANMembers` is read only by `/status`+`/interfaces`; the `configure-interface` write path uses the unchanged `vlanMemberPorts`. **Lesson:** on this VS host `bridged` is intermittently flaky (RCA-047 cold SAI convergence at exactly-1/3 bridge ports); confirm reproducibility (re-run) before attributing a failure to a diff, and never trust a degraded-host A/B — reboot first. Prior: 25/25 PASS **cold-deploy** 2026-07-14 (9m50s, feat/irb-service-redesign @ Phase C — the service-binding intent re-key to interface|<name>|service; apply/remove lifecycle produces clean state cold, confirming the DAG re-parenting + childless-identity teardown). Prior: 25/25 PASS **cold-deploy** 2026-07-10 (9m48s, feat/interface-kind-scenarios @ post-#432 + the TableKeys and QoS-teardown fixes). Suite grew to 25: 57-irb-bgp-peer (BGP Established ON Vlan200 over a bridged E16 domain + the update-irb in-place gateway-move device leg) and 59-service-on-portchannel (TRANSIT service applied ON PortChannel2 → service L3 in PORTCHANNEL_INTERFACE, session over the LAG, clean teardown); 56-portchannel gained canonical-LAG-table assertions (PORTCHANNEL_INTERFACE rows, zero INTERFACE|PortChannel residue) + acl-on-lag. The iteration to green surfaced and fixed two product bugs: ConfigDBClient.TableKeys returned transport-state-dependent key forms (prefixed when connected, entry keys from the projection fallback — now always entry keys per the documented contract), and RemoveService emitted an unconditional PORT_QOS_MAP delete that the narrowed LAG schema correctly refused (now binding-gated per §15). One cold run also hit the RCA-047-family orchagent starvation under 16-VM host load (first-boot factory-cleanup RIF-removal retry loop starves the vlan-member consumer; 46k retry lines) — did NOT reproduce on rerun; flake, evidence preserved in session scratchpad. Prior: 23/23 PASS **cold-deploy** in the 2026-07-07 sweep (9m10s). Prior: 23/23 cold (9m1s, 2026-07-06 against main @ post-#417, suite now 23 scenarios: the §48 dataplane continuity checks landed — 34-dataplane-update-bgp-peer (monotonic-uptime witness) and 36-dataplane-update-acl-rule (zero-loss drop-budget soak with a DROP backstop) — plus the #417 runner-edge adoptions live in the suite (cleanup: blocks, host-exec poll, device-scoped capture). Ran green cold 4× during the #415–#418 arc. The static-route and evpn-peer continuity checks do NOT live here: runtime STATIC_ROUTE creation needs the RCA-044 restart-replay and a second mid-suite bgp restart crash-loops bgp.service (static check → 2node-ngdp-primitive); the evpn witness is withdrawn entirely (RCA-049 addendum). Prior record: 21/21 PASS cold (7m26s, 2026-06-20 against main @ post-#261, Force10-S6000_vs, host shared with 3 labs / 19 VMs). First cold-deploy validation recorded against the global-platforms layout (#257 renamed the platform `sonic-vs` → `Force10-S6000_vs`; nodes updated). This run surfaced and fixed RCA-047: under heavy host contention a cold orchagent's SAI throughput slips past the `bridged` scenario's 60s ASIC poll; the fix (#261) raised that poll's budget to 180s (polls exit early on success, so warm runs are unaffected). **Process note (§42):** validate cold — `newtrun start` deploys the topology itself (lifecycle mode), so a warm re-run on an already-deployed lab masks the cold ASIC race and returns a false pass. Previously validated 21/21 on 2026-06-09 (5m47s, lighter host → the 60s budget held) against `feat/network-id-from-suite-topology` for #116. Validates the per-suite/per-lab network ID change: `newtlab deploy 2node-vs` registers newtron network as `id="2node-vs"` (not `"default"`); `newtrun start 2node-vs-primitive` derives `NetworkID="2node-vs"` from `suite.Network`; both reach the same registered slot end-to-end. Also surfaced a layering issue not captured in the original #116 design — newtlab-server held a single fixed-id newtron client at startup, so it couldn't serve per-lab IDs; fixed by changing `Config.NewtronClient` to `Config.NewtronClientFor func(networkID) SpecClient`, plus a fallback in `cmd/newtlab/main.go` `resolveTarget` for already-deployed labs whose `state.Dir` was empty. Previously validated 21/21 on 2026-06-07 against main @ 9139fd7 (post-#112 URL pluralization).
- `2node-vs-service`: VALIDATED — 7/7 PASS **cold-deploy** in the 2026-09-26 sweep (6m59s). Prior: 7/7 PASS 2026-09-24 (6m56s, fix/projection-partial-modify-merge @ #494, warm on the config-clean lab the primitive suite had just verified). Ran as the designated test for the **reverse** half of the §25 convergence: `deprovision` + `verify-clean` exercise RemoveService's `deleteACLIntent`/`deleteQoSBindingIntent`, and the provision leg goes through `topology-reconcile` rather than the primitives' scenario-level ops — the path the ledger records as catching teardown bugs the primitives structurally cannot. Prior: 7/7 PASS **cold-deploy** 2026-07-31 (7m1s, main @ post-#470). The strongest test of the default-on spec watcher (#468/#470): this suite provisions via `topology-reconcile`, which writes specs continuously while SSH sessions are live — exactly the interaction a spurious reload would break by draining node actors. Clean. Prior: 7/7 PASS **cold-deploy** 2026-07-19 (7m9s, fix/vrf-interface-mode-ipvpn-membership; first cold validation of this suite — prior records were warm). The provision path is `topology-reconcile` (unlike the primitives' scenario-level ops), so it exercised the standalone `bind-ipvpn` topology step (needed the `vrf_name` add) and the manual-IRB pre-provisioning (needed the per-service VRF-name fix — see the sweep note above); deprovision + verify-clean (with the corrected non-vacuous L3VNI-map check + the per-device VRF snapshot) all green. Prior: 7/7 PASS in the 2026-07-07 sweep (7m3s, fresh 2node-vs-service deployment, shared by the two drift suites below). Prior: 7/7 twice consecutively (run 1: 2026-06-07 21:33, 4m50s; run 2 confirmation: 2026-06-08 19:22, 4m41s; both against main @ ffe03eb + scenario fixes in this commit, bin/newt-server :18080). The other six scenarios (boot-ssh, provision, verify-health, dataplane, deprovision, verify-clean) have been validated many times and were not affected by the URL pluralization (#113). The 7th scenario, rollout-admin-status, was added 2026-06-04 in PR #74 as a demonstration of the parameterized-scenario feature and never validated end-to-end against the dependency chain; the post-pluralization run surfaced two gaps in its design: (1) no `requires:` field, so the topo-sort ran it before `provision` (writeIntent: parent "device" does not exist); (2) no reverse step for the admin_status property it sets (deprovision: deleteIntent has children — §15 violation). Both fixed in this commit. The CLAUDE.md "6/6 PASS, 2026-05-31" measurement predates the 7th scenario's addition by four days; this twice-confirmed 7/7 is the first record of the 7-scenario suite running clean end-to-end.
- `2node-ngdp-primitive`: VALIDATED — 22/22 PASS **cold-deploy** in the 2026-09-26 sweep (9m26s, CiscoVS). Prior: 22/22 PASS **cold-deploy** 2026-07-31 (10m10s, main @ post-#470, the CiscoVS half of the same sweep as the 2node-vs-primitive entry above — twelve PRs merged since 07-19, three runtime-affecting, the spec watcher now on by default). Silicon One exercises what the VS platform cannot: unified frrcfgd and the canonical-LAG assertions (PORTCHANNEL_INTERFACE rows, zero INTERFACE|PortChannel residue). Ran green **while a second lab (newtcon's 2node-vs) was provisioning concurrently** — the RCA-047 contention scenario that has historically flaked `bridged` cold ASIC convergence — so no contention question to answer this time. Prior: 22/22 PASS **cold-deploy** 2026-07-10 (9m59s, feat/interface-kind-scenarios; first try). 56-portchannel gained the canonical-LAG-table assertions (PORTCHANNEL_INTERFACE rows + zero INTERFACE|PortChannel residue — proving Silicon One intfmgrd programs from the yang-canonical table) and acl-on-lag (ACL_TABLE ports carries the LAG). Prior: 22/22 PASS **cold-deploy** in the 2026-07-07 sweep (9m53s). Prior: 22/22 cold (9m20s, 2026-07-06 against main @ post-#417, CiscoVS cisco-p200). Suite gained 35-dataplane-update-static-route (§48 zero-loss drop-budget soak; lives here because split of platform duties is empirical — see the 2node-vs entry) with the #417 cleanup:/poll adoptions. Ran green cold 3× during the #415–#418 arc. Note: these CiscoVS nodes run **unified frrcfgd** (`docker_routing_config_mode: unified` in the topology), not split bgpcfgd — RCA-044's split-mode framing does not describe this repo's fabrics (verified during RCA-049).
- `3node-ngdp-dataplane`: VALIDATED — 8/8 PASS **cold-deploy** in the 2026-09-26 sweep (8m54s). Prior: 8/8 PASS **cold-deploy** in the 2026-07-07 sweep (8m54s). Prior: 8/8 cold (9m4s, 2026-07-06 against main @ post-#418, with the RCA-049 fixes live: canonical-struct decode in the update handlers + the BGPNeighborExists projection leg). This fabric's overlay is **leaf1↔leaf2 by design** (nodes/leaf{1,2}.json `evpn.peers`; the spine is underlay-only transit with NO VTEP/loopback overlay) — do not author tests that add/remove overlay peers here; every loopback pair is profile-owned and AddBGPEVPNPeer now refuses adoption (§27).
- `1node-vs-basic`: VALIDATED — 9/9 PASS in the 2026-09-26 sweep (7m44s, cold-deployed 1node-vs lab) — **after fixing a stale scenario the sweep caught.** `05-configdb-snapshot-actuated` ran `configdb snapshot` with no flag while asserting `.DEVICE_METADATA == null`; the CLI default had flipped to return the entire CONFIG_DB (`--owned-only` is opt-in now) and the scenario never caught up — its own description still called owned-only "the default". DEVICE_METADATA *is* in `excludedFromDrift`, so the assertion was right and the command had lost its flag. Fixed by adding `--owned-only` to the two owned-scope steps. Also note this suite's scenarios are slow (perwrite/service-projection/projection-diff are ~2m20s each) — a run in progress looks stalled; check for the summary line before assuming it finished. Prior: 9/9 PASS (7m41s, 2026-07-07 sweep, on the running 1node-vs lab). First ledger record for this suite.
- `2node-vs-drift`: VALIDATED — 7/7 PASS in the 2026-09-26 sweep (10m15s, warm on the cold 2node-vs-service deployment) — **after fixing a stale teardown the sweep caught.** `12-teardown` removed services from `Ethernet16` and `Ethernet4`, but the irb-service-redesign flip (7fd5cb7) moved both irb services onto their VLAN interfaces: topology.json applies eirb1/eirb2 on Vlan400/Vlan401 and irb on Vlan100, and the member ports carry no binding (`500: interface Ethernet16 has no service to remove`). The service suite's own deprovision was updated at the time; this teardown was missed, and the 07-07 record below **predates the flip by a week**, so no sweep had run it since. Fixed the four delivery points; the 10m15s matches its pre-flip timing exactly. Prior: 7/7 PASS (10m15s, 2026-07-07 sweep, warm on the 2node-vs-service deployment; the reprovision scenario dominates at 4m40s). First ledger record.
- `2node-vs-drift-actuated`: VALIDATED — 8/8 PASS in the 2026-09-26 sweep (6m58s, same deployment). Prior: 8/8 PASS (5m36s, 2026-07-07 sweep, same deployment). First ledger record.
- `simple-vrf-host`: VALIDATED — 5/5 PASS in the 2026-09-26 sweep (7s, warm on the 2node-ngdp lab straight after the primitive suite). Prior: 5/5 PASS (7s, 2026-07-07 sweep, warm on the 2node-ngdp lab straight after the primitive suite). First ledger record.
- `2node-ngdp-service`: VALIDATED — 6/6 PASS **cold-deploy** in the 2026-09-26 sweep (7m48s, CiscoVS). Prior: 6/6 PASS **cold-deploy** 2026-07-31 (7m59s, main @ post-#470; CiscoVS half of the same service-suite pair). **Session-TTL trap — read this before a sweep:** the first attempt reported 3 failures that were NOT a regression. A session key lives 8h and dies mid-run without warning: scenarios 1–2 passed, then everything after 22:05:25 returned `401: invalid or expired session key`, and a host step failed with `SSH session: EOF` as collateral against the half-verified device. The tell that it was environmental: 2node-vs-service passed 7/7 on identical code seven minutes earlier. Re-minting the session and re-running cold gave 6/6. **Mint a fresh session immediately before a sweep** rather than reusing whatever is cached — an already-old session survives the first suite and dies in the tail, where it looks like a platform-specific failure. (A server restart also drops in-memory sessions.) Prior: 6/6 PASS **cold-deploy** 2026-07-19 (7m59s, fix/vrf-interface-mode-ipvpn-membership; first cold validation on CiscoVS — same topology fixes as the vs-service suite: standalone `bind-ipvpn` vrf_name + manual-IRB per-service VRF name + corrected non-vacuous L3VNI-map check). Prior: 6/6 PASS (7m57s, 2026-07-07 sweep, fresh 2node-ngdp-service deployment, warm). First CLAUDE.md ledger record (previously noted only as "6/6, Feb 2026" in session memory).
- `1node-vs-auth`: VALIDATED — 36/37 PASS / 0 FAIL / 1 SKIP (L2c-round-trip, by design) in the 2026-09-26 sweep (<1s, loopback). Prior: 36/37 PASS / 0 FAIL / 1 SKIP (L2c-round-trip by design) in the 2026-07-07 sweep (<1s; the suite grew by one scenario since the 36-scenario records below). Prior: 35/36; re-run **2026-07-01 against main @ post-#348** (per-file zones #346 + node/zone CLI `-x` #347) — confirms per-file zones (`zones/<zone>.json`) cause no regression on the full L0→L6 auth arc; ran twice consistently, seconds each (loopback), under the running `--enforce-authorization --auth-pam-service newtron-test --audit --audit-integrity --spec-watch` server (extra global `--super-users aldrin,ron --dev-superuser=false` is harmless — `root` is super via the network's own `super_users:[root]`). Needs `newtron` on the server's PATH for the L6 `newtron-cli` audit-verify exec (`ln -sf "$PWD/bin/newtron" ~/go/bin/newtron`; see the config-suite caveat above). Original 2026-06-20 against main @ post-#255, `bin/newt-server --networks-base ... --platforms-base ... --audit --auth-pam-service newtron-test --enforce-authorization --audit-integrity --spec-watch`, completes in seconds (loopback). This run validated the IPVPN/VRF-name collapse follow-up (#255): the suite's 75-L4-vrf-permission-families scenarios were updated to the single-name bind-ipvpn wire shape (IPVPN name IS the SONiC VRF name, `^Vrf[A-Za-z0-9_]*$`), and a stale `/profiles` → `/nodes` route in 00-L0-secret-store-resolves (which had been 404-ing and skipping the whole dep chain) was fixed in the same PR. Requires `--audit` set for the L4-audit-read-gated scenarios; without it they 404. (Flag renamed from `--audit-log PATH` in #341 — audit is per-network now, no path.) Prior baseline: 32/33 on 2026-06-16 against main @ post-#192. **First end-to-end validation of this suite in the project's history** — the suite was authored in PR #139 (May 2026) but the actual run that PR #182 forced surfaced multiple pre-existing bugs (canonical-form where-patterns + the `bin/newtrun start` Bearer-forwarding gap closed in PR #186) plus added 5 new scenarios via PR #181 + 3 via PR #188. The 33 scenarios cover the full L0→L6 arc plus the new L4-vrf-families (vrf.bind/vrf.route/bgp.peer), L5-service-dimension (where:{service:"X"} on gateService-routed permissions), and L4-auth-read-gated (engage-when-configured `auth.read` permission). Reproducible via the 3-command operator-friendly recipe in `networks/1node-vs-auth/suites/1node-vs-auth/README.md` §Validation.
- `1node-vs-auth-deployed`: VALIDATED — 3/3 PASS in the 2026-09-26 sweep (<1s, on the cold-deployed 1node-vs lab). Prior: 3/3 PASS in the 2026-07-07 sweep (<1s, on the running 1node-vs lab). Prior: 3/3, re-run **2026-06-30 against main @ post-#343** (the per-network audit arc #340–#343 landed) with `bin/newt-server --enforce-authorization --auth-pam-service newtron-test --audit --audit-integrity --super-users aldrin,ron --dev-superuser=false --spec-watch`, <1s on the already-deployed 1node-vs lab; first validated 3/3 on post-#336 (#337). Added in #337 as the **actuated** counterpart to `1node-vs-auth`: where that suite is loopback (`--no-deploy`, every scenario `?mode=topology` so the gate fires before any transport) and therefore never crosses the newtron→newtlab boundary, this one runs WITHOUT `--no-deploy` so device ops force SSH-port resolution from newtlab over HTTP loopback under PAM — the exact cross-engine path #335's internal service token fixed, previously guarded by nothing. Three scenarios: `00-cross-engine-ssh` (as ron, actuated `ssh-command echo ok` round-trips → internal newtlab lookup authenticated; a regressed service token surfaces as `newtlab (401)`), `10-cross-engine-health` (as ron, actuated `GET /health` connects + returns `oper_checks`), `20-enforcement-live` (as mallory/ungranted, actuated write DENIED → enforcement live, so 00/10 aren't vacuous). Non-mutating on the device (00/10 read; 20 denied pre-transport) and `EnsureTopology` reuses a running lab, so it's safe against an already-provisioned switch1 and doesn't need a cold deploy. Cache identities first: `sh networks/1node-vs/suites/1node-vs-auth-deployed/login-users.sh`; run `NEWTRON_USER=ron bin/newtrun start 1node-vs-auth-deployed`. Recipe in that suite's README.md §Validation. **Post-#343 the run also confirms per-network audit end-to-end on the actuated path:** the 4 events (ron ALLOW `ssh-command` + `authcheck:device.write`; mallory DENY `create-vlan` + `authcheck:vlan.create`) all land in `networks/1node-vs/audit/audit.log` with `network=1node-vs`, and `bin/newtron -N 1node-vs audit verify` reports the hash chain clean (4 entries) — proving both mutation and decision events route to the network's own log, not just under loopback. (Cross-engine calls in newt-server are HTTP loopback, not in-process Go calls — the #335 401 is the proof; #337 also corrected the auth-design.md/newt-server.md passages that still claimed in-process.)
