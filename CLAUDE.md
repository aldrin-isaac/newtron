...
- `2node-vs-primitive`: VALIDATED — 21/21 PASS (re-run 2026-06-06 with bin/newt-server on :18080, 7m24s end-to-end in lifecycle mode). Validates the #101 stack: lockManager (PR #102), engine-layer atomic methods + per-key locks (PR #103), API-layer lock strip (PR #104). The API layer now holds no spec lock at all — every public Network method is engine-atomic via keyNetworkSpec / keyTopology / keyNodes. The earlier loopback deadlock that drove issue #97 is structurally impossible. Previously validated 21/21 on 2026-06-05 against the post-PR-96 code; this re-run confirms the post-PR-104 architecture preserves the working behavior end-to-end.
