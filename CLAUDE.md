...
- `1node-vs-config`: VALIDATED — 25/25 PASS (loopback `--no-deploy`, ~2s, 2026-06-20 against main @ 7560efd). Re-run green at every step of the register-network → IPVPN/VRF-collapse → global-platforms arc (#245, #251, #249, #250, #252, #253, #256, #257, #258, #259). The suite grew from the original 13 to 25 scenarios; this is the current full set. Loopback mode has no cold-deploy/ASIC-convergence exposure (no real switch), so it's the fast regression gate for spec/wire-shape changes. Caveat: clear stale auth sessions first if a prior `1node-vs-auth` run cached per-user sessions on the same port — the CLI bails with "multiple cached sessions" otherwise.
