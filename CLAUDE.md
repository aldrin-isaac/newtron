...
- `1node-vs-config`: VALIDATED — 25/25 PASS (loopback `--no-deploy`, ~2s); re-run **2026-06-30 against main @ post-#344** (the per-network audit arc #340–#343 landed) as ron under the enforced + `--audit`-enabled server; original 2026-06-20 against main @ 7560efd. Re-run green at every step of the register-network → IPVPN/VRF-collapse → global-platforms arc (#245, #251, #249, #250, #252, #253, #256, #257, #258, #259). The suite grew from the original 13 to 25 scenarios; this is the current full set. Loopback mode has no cold-deploy/ASIC-convergence exposure (no real switch), so it's the fast regression gate for spec/wire-shape changes. **Session caveat (the exact mechanism):** the suite uses the `newtron-cli` action, which newt-server execs **server-side** — inheriting newt-server's env (so `NEWTRON_USER` is unset unless the server was launched with it) and passing `--server http://127.0.0.1:18080` but **not** `--user`. So it can't disambiguate when MULTIPLE sessions are cached and bails with "multiple cached sessions" (symptom: `setup-device` FAILs → 14 skips, ~9/2/14 — looks like a regression but isn't). This bites right after running `1node-vs-auth-deployed`, which caches ron **and** mallory. Fix: clear `~/.newtron/sessions/` down to the single `ron@127.0.0.1_18080.json` (matching the exec's `--server` host), or launch newt-server with `NEWTRON_USER=ron` in its env.
