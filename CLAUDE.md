...
- `2node-vs-primitive`: VALIDATED — 21/21 PASS (re-run 2026-06-05 with bin/newt-server on :18080, 6m8s end-to-end in lifecycle mode). Re-run exercised PR #95 (2node-vs topology source_ip fix for `add-bgp-evpn-peer`) and surfaced a latent newt-server deadlock: NetworkActor.do() funnels every request through one goroutine, and newtlab.NewLab's loopback HTTP for spec data (topology / platforms / profile) queued behind any in-flight actor closure (e.g. `handleGetHostProfile` resolving an SSH port). The inner http.Client timed out at 30s every time. PR #96 bypasses the actor for the three read-only spec endpoints; issue #97 tracks the structural fix.
