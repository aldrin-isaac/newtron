...
- `1node-vs-config`: VALIDATED — 25/25 PASS (loopback `--no-deploy`, ~2s); re-run **2026-07-01 against main @ post-#347** (per-file zones #346 + the `node`/`zone` CLI `-x` fix #347 landed) as ron under the enforced + `--audit`-enabled server — confirms per-file zones (`zones/<zone>.json`) cause no regression on the CLI-config path; prior re-run 2026-06-30 against main @ post-#344 (the per-network audit arc #340–#343 landed); original 2026-06-20 against main @ 7560efd. Re-run green at every step of the register-network → IPVPN/VRF-collapse → global-platforms arc (#245, #251, #249, #250, #252, #253, #256, #257, #258, #259). The suite grew from the original 13 to 25 scenarios; this is the current full set. Loopback mode has no cold-deploy/ASIC-convergence exposure (no real switch), so it's the fast regression gate for spec/wire-shape changes. **Session caveat — RESOLVED by identity forwarding:** the suite uses the `newtron-cli` action, which newt-server execs **server-side**. It *used to* inherit only newt-server's env (no `NEWTRON_USER`) and pass `--server` but **not** `--user`, so the exec'd CLI fell back to the session-file cache and bailed with "multiple cached sessions" whenever more than one identity was cached (symptom: `setup-device` FAILs → 14 skips, ~9/2/14 — looked like a regression but wasn't; it bit right after `1node-vs-auth`/`1node-vs-auth-deployed` cached many users). **Fixed:** the runner now forwards the run's operator Bearer (or a scenario's `as:` user's Bearer) to the exec via the `NEWTRON_BEARER` env var — the same identity it forwards on its HTTP calls (`Runner.scenarioBearer`, steps_cli.go) — so the exec authenticates as that identity **without reading `~/.newtron/sessions/` at all**. The number of cached sessions no longer matters; multi-user/multi-session is honored. (Obsolete workarounds, no longer needed: clearing sessions to a single one, or launching with `NEWTRON_USER=ron`.) **Second mechanism, same 9/2/14 symptom:** the server-side exec resolves the `newtron` binary from the newt-server process's `$PATH` (steps_cli.go). If newt-server was launched (e.g. via `nohup bin/newt-server …`) with a `$PATH` that lacks the repo `bin/`, every `newtron-cli`-action step fails with `exec: "newtron": executable file not found in $PATH` — the 9 `action: newtron` (in-process/HTTP) scenarios still pass, the 2 `action: newtron-cli` scenarios (setup-device, topology-show) fail, and their 14 dependents skip → the same 9/2/14. Fix: put the freshly-built binary on the server's PATH (`ln -sf "$PWD/bin/newtron" ~/go/bin/newtron` — `~/go/bin` is already on PATH; the exec resolves fresh per call, no restart needed). Diagnose the split with `--junit out.xml`, which carries the per-step failure message the streaming summary omits.
