...
- `2node-vs-primitive`: VALIDATED — 21/21 PASS (re-run 2026-06-07 with bin/newt-server on :18080, 8m12s end-to-end in lifecycle mode against main @ 9139fd7). Validates the post-pluralization (#112) URL surface: every collection path now plural (`/networks`, `/nodes/{d}/interfaces`, `/nodes/{d}/routes`), every action verb still singular. One straggler — `/nodes/{{device}}/route/default/...` in three `32-cross-switch.yaml` scenario files — surfaced only at run time and is fixed in this commit. Also validates the post-#110 scaffold endpoint (newtlab deploy registers networks via `POST /newtron/v1/networks`) and the post-#111 newtlab `/labs` rename. Previously validated 21/21 on 2026-06-06 against post-#101 main @ ec16631.
