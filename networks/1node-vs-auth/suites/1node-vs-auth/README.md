...
| `75-L4-vrf-permission-families` | L4 | Split into `-mallory` + `-alice-setup` + `-bob` + `-alice-cleanup`. The three families from the #170 vrf.modify/evpn.modify split that 70-L4 left as TODO: `vrf.bind` (bind-ipvpn/unbind-ipvpn), `vrf.route` (add-static-route/remove-static-route), `bgp.peer` (interface add-bgp-peer/remove-bgp-peer). Mallory's denies need no fixtures (gate fires before lookup); bob's allows use the IP-VPN spec `BOB` (added by alice-setup), whose derived on-device VRF name is `Vrf_BOB` — the VRF bob created in 30-L4. Bob also exercises every reverse so the projection ends clean; alice cleans up the IP-VPN spec last. |
