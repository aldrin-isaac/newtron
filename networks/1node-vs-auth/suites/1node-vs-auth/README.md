...
| `75-L4-vrf-permission-families` | L4 | Split into `-mallory` + `-alice-setup` + `-bob` + `-alice-cleanup`. The three families from the #170 vrf.modify/evpn.modify split that 70-L4 left as TODO: `vrf.bind` (bind-ipvpn/unbind-ipvpn), `vrf.route` (add-static-route/remove-static-route), `bgp.peer` (interface add-bgp-peer/remove-bgp-peer). Mallory's denies need no fixtures (gate fires before lookup); bob's allows use `Vrf_bob` (the VRF created in 30-L4 and the IPVPN spec alice-setup adds under the same name — the IPVPN name IS the SONiC VRF name post-#253). Bob also exercises every reverse so the projection ends clean; alice cleans up the IPVPN spec last. |
...
| `85-L5-service-dimension` | L5 | Single-identity (`-svc-sam`) coverage for the **service** `where:` dimension — reachable only via `Interface.gateService` (apply-service / remove-service / refresh-service). Three pins: (1) svc-sam in service-team granted `service.apply` with `where:{service:"transit-*"}` can apply transit-1 (gateService stamps Context.Service="transit-1", matches); (2) applying vpn-east is denied (Context.Service="vpn-east" doesn't match transit-*; gate fires before the missing-service-spec lookup); (3) svc-sam is also granted `vrf.create` with the same where clause — denied on create-vrf because Node.gate doesn't stamp Context.Service, so the where clause has nothing to match, pinning the dimension-unreachable fail-closed behavior for non-service-bound permissions. |
...
sh networks/1node-vs-auth/suites/1node-vs-auth/login-all.sh
