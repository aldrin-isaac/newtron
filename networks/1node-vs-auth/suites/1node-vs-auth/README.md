...
| `75-L4-vrf-permission-families` | L4 | Split into `-mallory` + `-alice-setup` + `-bob` + `-alice-cleanup`. The three families from the #170 vrf.modify/evpn.modify split that 70-L4 left as TODO: `vrf.bind` (bind-ipvpn/unbind-ipvpn), `vrf.route` (add-static-route/remove-static-route), `bgp.peer` (interface add-bgp-peer/remove-bgp-peer). Mallory's denies need no fixtures (gate fires before lookup); bob's allows use vrf-bob (from 30-L4) and ipvpn-75 (alice-setup creates it). Bob also exercises every reverse so the projection ends clean; alice cleans up the IPVPN spec last. |
