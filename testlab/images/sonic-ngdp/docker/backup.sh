...
handle_args() {
    ...
    # Use container's management IP instead of localhost to avoid QEMU SLiRP
    # CLOSE-WAIT bug. Connections via the management IP route through Docker
    # bridge networking rather than the SLiRP loopback stack.
    MGMT_IP=$(ip -4 addr show eth0 | grep -oP '(?<=inet\s)\d+\.\d+\.\d+\.\d+' | head -1)
    HOST="$user@${MGMT_IP:-localhost}"
