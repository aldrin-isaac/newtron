...
func main() {
	...
	specWatch := flag.Bool("spec-watch", true, "watch every registered network's network directory for file changes on the newtron engine; on settled change (1s debounce) automatically reload the network so an edited grant table takes effect without an explicit /reload call. A reload whose specs are unchanged is a no-op, so newtron's own spec writes cost nothing. Set =false only where the filesystem cannot deliver change notifications (some network mounts) or inotify watches are exhausted. (auth-design.md L6)")
