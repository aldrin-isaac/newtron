...
NETWORK_DIR="networks/1node-vs"  # for displaying spec content; newt-server auto-discovers
...
echo ""
echo -e "  ${BOLD}nodes/switch1.json${RESET} identifies the device -- its ASN, loopback,"
echo "  management IP, and credentials:"
echo ""
echo -e "  ${GRAY}$NETWORK_DIR/nodes/switch1.json${RESET}"
cat "$NETWORK_DIR/nodes/switch1.json" | sed 's/^/    /'
echo ""
