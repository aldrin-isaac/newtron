...
**Note:** System MAC is no longer handled by a boot patch. The factory CONFIG_DB
includes a MAC, which survives provisioning via merge-based `ReplaceAll` (RCA-035) —
the composite does not overwrite `DEVICE_METADATA|localhost.mac` when no MAC is
supplied, so the factory value remains.
