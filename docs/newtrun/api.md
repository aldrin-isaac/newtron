...
Defaults to `http://127.0.0.1:8081`. Override at server start with `--listen <addr>`; clients pass the URL via `--newtrun-server <url>` or `NEWTRUN_SERVER`.
...
- Response bodies: `application/json` for everything except `GET /api/suites/{suite}/scenarios/{name}` (raw YAML, `application/yaml`) and `GET /api/runs/{suite}/events` (Server-Sent Events, `text/event-stream`).
...
A non-200 from `/api/health` means newtrun-server is not the process answering on that port (or is mid-shutdown). The CLI's `requireServer` probe uses this endpoint to decide whether to print "newtrun-server is not running".
...
|--------|------|--------|---------|
| `POST` | `/api/runs` | 202 / 404 / 409 | Start or resume a run |
| `GET` | `/api/runs` | 200 | List runs visible on disk |
| `GET` | `/api/runs/{suite}` | 200 / 404 | Read one run's full state |
| `DELETE` | `/api/runs/{suite}` | 200 / 409 | Remove a terminal run's state |
| `POST` | `/api/runs/{suite}/pause` | 200 / 404 | Request graceful pause |
| `POST` | `/api/runs/{suite}/stop` | 200 / 404 | Cancel the runner's context |
| `GET` | `/api/runs/{suite}/events` | 200 (SSE) | Subscribe to progress events ([§5](#5-run-events-sse)) |

### `POST /api/runs` — start or resume a run
...
### `DELETE /api/runs/{suite}` — remove state
...
### `POST /api/runs/{suite}/pause` — graceful pause

Writes `state.Status = pausing` to disk. The Runner picks up the pause signal at the next scenario boundary via `CheckPausing` and exits with `PauseError`. The eventual `state.Status` becomes `paused`. A subsequent `POST /api/runs` for the same suite resumes from the next unprocessed scenario.
...
### `POST /api/runs/{suite}/stop` — cancel runner
...
**Late-subscribe race:** if a run completes before the client subscribes, no events arrive at all. The client should fall back to `GET /api/runs/{suite}` to read the terminal state. The CLI's `start` command tracks whether `SuiteEnd` was ever seen and treats "no SuiteEnd" as an infrastructure error (exit 2 with the "connection lost" message).
...
The `run_id` becomes the `{suite}` path parameter for subsequent calls to `/api/runs/{suite}`, `/events`, etc. The inline and suite namespaces share the same handler routes — the server resolves both via `LoadAnyRunState`.
...
| Method | Path | Status | Purpose |
|--------|------|--------|---------|
| `GET` | `/api/suites` | 200 | List suite directories |
| `POST` | `/api/suites` | 201 / 409 | Create an empty suite |
| `DELETE` | `/api/suites/{suite}` | 204 / 404 / 409 | Delete an empty suite |
| `GET` | `/api/suites/{suite}/scenarios` | 200 / 404 | List scenarios in a suite |
...
### `GET /api/suites/{suite}/scenarios`
...
|--------|------|-------------|--------|
| `GET` | `/api/suites/{suite}/scenarios/{name}` | `application/yaml` (response) | 200 / 404 |
| `PUT` | `/api/suites/{suite}/scenarios/{name}` | `text/*` (request) | 200 / 201 / 400 |
| `DELETE` | `/api/suites/{suite}/scenarios/{name}` | — | 204 / 404 |

The PUT path is the one with real behavior; the other two are uniform.

### `GET /api/suites/{suite}/scenarios/{name}` — read raw YAML
...
### `PUT /api/suites/{suite}/scenarios/{name}` — create or update
...
### `DELETE /api/suites/{suite}/scenarios/{name}`
...
The stream from `/api/runs/{suite}/events` carries seven event types in this order:
...
The complete record for one run. Returned by `GET /api/runs/{suite}` ([§4](#4-suite-backed-run-lifecycle)).
...
The list-view summary of a run. Returned by `GET /api/runs` ([§4](#4-suite-backed-run-lifecycle)).
...
The per-scenario summary used in `suite_start` events and `GET /api/suites/{suite}/scenarios`.
...
*This document was source-traced against `pkg/newtrun/api/server.go` (route table), `runs.go`, `suites.go`, `scenarios.go`, `topologies.go`, and `types.go`. Every endpoint claim was verified by reading the handler. If you find a discrepancy, the code is the authority — please open an issue or PR.*
