...
newtrun is an E2E test orchestrator for newtron and SONiC. It parses YAML scenario files, deploys VM topologies via newtlab, provisions devices via newtron, and runs multi-step verification sequences. This document covers `pkg/newtrun/` and `cmd/newtrun/`.

For the high-level architecture, see [newtrun HLD](hld.md). For the device connection layer, see [Device Layer LLD](../newtron/device-lld.md).
...
| 9 | `verifyPingExecutor` | `verify-ping` | SSH ping (newtrun native) |
...
|------|------|---------------|
| `InfraError` | Deploy fails, SSH connect fails | `"newtrun: deploy: <err>"` or `"newtrun: connect leaf1: <err>"` |
...
```go
func main() {
    rootCmd := &cobra.Command{Use: "newtrun", ...}
...
are resolved under `newtrun/suites/` or used as paths directly.
