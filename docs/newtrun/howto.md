...
newtrun is an E2E testing orchestrator for newtron and SONiC. It deploys
...
newtrun depends on newtlab for VM management. Ensure newtlab is set up:
...
newtrun ships with pre-defined topologies in `newtrun/topologies/`. These are
...
```
newtrun/topologies/4node/specs/
├── topology.json
├── network.json
├── platforms.json
└── profiles/
...
They live in `newtrun/suites/` — either `2node-standalone/` (independent tests)
...
```yaml
# newtrun/suites/2node-standalone/my-test.yaml
name: my-test
...
```bash
newtrun start 2node-standalone --scenario my-test
```
...
newtrun determines the expected values from the topology spec — newtron's
...
```yaml
# newtrun/suites/2node-incremental/07-vrf-lifecycle.yaml
name: vrf-lifecycle
...
The suite name resolves to `newtrun/suites/2node-incremental/`. You can also
pass a full path with `--dir`:

```bash
newtrun start --dir newtrun/suites/2node-incremental
```
...
newtrun start 2node-incremental --platform sonic-vs
```
...
```bash
newtrun start 2node-incremental --topology 4node
```

### Verbose Output

```bash
newtrun start 2node-incremental -v
```
...
newtrun manages suite runs as stateful lifecycles. State is persisted at
`~/.newtron/newtrun/<suite>/state.json` so you can pause, resume, and monitor
...
newtrun detects the paused state and resumes from where it left off. Already
...
# "paused after 3/5 scenarios; resume with: newtrun start 2node-incremental"
...
```
newtrun: 5 scenarios, topology: 2node, platform: sonic-vpp
...
---
newtrun: 5 scenarios: 4 passed, 1 failed  (1m06s)
...
`newtrun/.generated/report.md` with a summary table and failure/skip details.
...
Data plane tests require host endpoints that can generate and receive traffic. newtrun uses **host** devices (Alpine Linux VMs) that are provisioned by newtlab during topology deployment. Each host device runs in its own network namespace, eliminating the need for runtime namespace management in test scenarios.
...
newtron provision -S newtrun/topologies/2node/specs/ -d leaf1 -x -v
```
...
newtrun start 2node-standalone --scenario health -v
...
```
newtrun start 2node-incremental                        # all scenarios
newtrun start 2node-incremental --scenario boot-ssh    # single scenario
