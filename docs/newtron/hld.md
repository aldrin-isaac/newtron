...
**Current enforcement status:** the entitlement pattern is wired live as of L3. L0 secret store is shipped (`${secret:KEY}` references in `nodes/*.json` / `platforms.json` resolved at network load when `--secret-store=PATH` is set, or auto-discovered from `<spec-dir>/secrets.json` when no flag is set; see [`secret-store.md`](secret-store.md)). The in-repo test networks under `networks/` ship per-network `secrets.json` files (mode 0600) so the README quickstart command runs zero-ceremony (#176). L1 audit log infrastructure is shipped (mutation events recorded via `--audit-log=PATH` with caller identity from Unix peer creds, self-attested header, or none). L2a listener-side TLS is shipped — the cert-CN extraction infrastructure (`pkg/httputil/conn_creds.go` + caller-middleware priority slot) plus the listener-side flags (`--tls-cert`/`--tls-key`/`--tls-ca` on `cmd/newt-server`) are all wired (#175). When all three are set, every TCP connection completes a mTLS handshake; the verified peer cert CN flows into the caller-middleware identity slot ahead of PAM and the audit log records `verification_source: "service_cert_cn"`. Deployments that prefer reverse-proxy termination continue to do so (see [`mtls-howto.md`](mtls-howto.md)). L2b user-to-service PAM is shipped — `--auth-pam-service=NAME` enforces HTTP Basic + `pam_authenticate` on TCP requests; audit log records `verification_source: "pam"`; see [`pam-howto.md`](pam-howto.md). L3 authorization enforcement is shipped — `--enforce-authorization` engages `Network.EnableAuthorization` at every `RegisterNetwork`/`ReloadNetwork`. L4 coverage closure is shipped — every public mutation method on `*Network`, `*Node`, and `*Interface` calls a permission gate before any state change; constants extend the verb family with `acl.create`/`acl.delete` (symmetry) and `device.write` (operational catch-all). `TestAPICompleteness` refuses to compile a new mutation surface without an explicit permission classification. L5 fine-grained per-resource grants is shipped — `network.json` permission values accept the typed `[{groups, where}]` form alongside the legacy `[group]` shorthand; the `where` clause scopes a grant on `device`/`service`/`interface`/`resource`/`field` dimensions with a single matcher supporting exact/glob/comma-OR/bang-exclusion. The `field` dimension implements the §3 criterion 9 meta-authorization separation — `spec.author` can be scoped away from the `permissions`/`user_groups`/`super_users` fields so an IAM operator role is distinct from a service-architect role. Denials surface as HTTP 403 with the typed `AuthorizationError` payload, and per-decision audit events (`authcheck:<permission>`) record `device`/`service`/`interface`/`resource`/`field` for forensic reconstruction when both `--audit-log` and `--enforce-authorization` are set; see [`authorization-howto.md`](authorization-howto.md). Without the flag, pre-enforcement behavior is preserved (checks return nil) per auth-design.md §2.4. L6 operational hardening is shipped — `--spec-watch` engages an fsnotify watcher on every registered network's spec dir; on settled change (1s debounce) it auto-reloads the network so revoked grants take effect without a `/reload` call. `--audit-log-integrity` hash-chains every emitted audit event (Event.ID = SHA256(prev_hash || canonical_json)); `bin/newtron audit verify <log-path>` walks the chain and reports the first broken link with exit code 1 + the tampered line number. The chain head is recovered from the file's last well-formed entry on restart so multi-lifecycle logs verify as one chain. The auth arc (L0–L6) is complete.

**Layered path to production-grade.** The design destination is the existing entitlement pattern, extended to meet audit requirements. The path is seven layers; each layer ships as one PR and closes a defined security gap independently. [`auth-design.md`](auth-design.md) is the L0 deliverable: it articulates the threat model (in-scope and out-of-scope on the operator-to-server channel), the goal-state criteria a security review must be able to verify, and L1–L6 with their dependencies. L1 wires up the audit log so accountability is answerable today (with verified identity for the Unix socket path; self-attested-but-labeled for TCP until L2). L2 splits into L2a (listener-side TLS) and L2b (PAM); they're independent and shippable in either order. L3 turns the entitlement checks live; L4 closes coverage to Node operations; L5 adds fine-grained per-device grants; L6 closes operational gaps (revocation, secret hygiene, log integrity). Identity providers beyond PAM (OIDC, SAML, bearer tokens) are explicitly out of scope — operators integrate via PAM modules instead (`pam_ldap`, `pam_sss`, `pam_krb5`).
