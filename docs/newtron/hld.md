...
**Current enforcement status:** the entitlement pattern is wired into 26 mutation paths in `spec_ops.go` and `profile_ops.go`, but `Network.SetAuth` is never called at server startup — so `net.auth` is always nil, and every check short-circuits to "allowed." Authorization is not enforced. L0 secret store is shipped (`${secret:KEY}` references in `profiles/*.json` / `platforms.json` resolved at network load when `--secret-store=PATH` is set; see [`secret-store.md`](secret-store.md)). L1 audit log infrastructure is shipped (mutation events recorded via `--audit-log=PATH` with caller identity from Unix peer creds, self-attested header, or none). L2a inter-service mTLS is shipped — three flags per standalone engine binary (`--tls-cert`, `--tls-key`, `--tls-ca`); when configured, cross-process calls between newtron / newtlab / newtrun engines run over mTLS and the audit log records `verification_source: "service_cert_cn"`. See [`mtls-howto.md`](mtls-howto.md). L2b user-to-service PAM and authorization enforcement (L3) are not yet shipped. Treat user-facing endpoints as unauthenticated and deploy behind a trusted-network boundary until those layers land.
