...
**Note (Feb 2026):** The `ApplyFRRDefaults` mechanism has been eliminated. FRR configuration is now managed by `frrcfgd` (unified mode) via a patched `frrcfgd.py.tmpl`. The timing and idempotency issues described here no longer apply. The `2node-ngdp-primitive` suite has been replaced by `2node-ngdp-primitive`.

**Platform**: Platforms using bgpcfgd in split mode (e.g., sonic-vs default). CiscoVS uses frrcfgd unified mode and is not affected by this specific issue.
**Component**: bgpcfgd (FRR config daemon), `newtrun/topologies/2node-ngdp/suites/2node-ngdp-primitive/08-evpn-setup.yaml`
