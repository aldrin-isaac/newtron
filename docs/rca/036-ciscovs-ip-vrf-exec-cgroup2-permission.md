...
`newtest/suites/2node-primitive/01-provision.yaml` — `prime-ngdp-arp-customer-vrf` step:
```bash
# Before (broken):
...
`newtest/suites/2node-primitive/35-vrf-routing.yaml` — `switch2-prime-arp` step:
```bash
# Before (broken):
