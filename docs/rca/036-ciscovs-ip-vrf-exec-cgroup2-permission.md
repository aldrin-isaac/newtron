...
`newtest/suites/2node-incremental/01-provision.yaml` — `prime-ngdp-arp-customer-vrf` step:
```bash
# Before (broken):
...
`newtest/suites/2node-incremental/35-vrf-routing.yaml` — `switch2-prime-arp` step:
```bash
# Before (broken):
