...
**Note (Feb 2026):** This factory config defect is harmless in practice. The topology provisioner overwrites the factory BGP config during provisioning, eliminating the invalid `local-as` entry. The crash only affects unprovisioned first boot of the raw image.
...
The factory CiscoVS image ships with a pre-baked CONFIG_DB containing a sample BGP
neighbor with `local_asn` matching the router's own ASN. The config is generated by
`sonic-cfggen` using `bgpd.conf.db.j2` from this CONFIG_DB seed data.

This is a factory image defect — newtron's topology provisioner overwrites the factory
BGP config entirely, so the invalid entry never survives into production. The crash
only occurs on unprovisionied first boot of the raw CiscoVS image.
