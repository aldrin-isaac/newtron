// Package operations implements device configuration operations with strict validation.
package operations
...
// Operation defines the interface for all configuration operations.
// Operations receive a *network.Device which has parent reference to Network,
// allowing access to all configuration through the object hierarchy.
type Operation interface {
	// Validate checks all preconditions before execution
	// Returns error if any precondition is not met
	Validate(ctx context.Context, d *network.Device) error

	// Preview returns the changes that would be made without applying them
	Preview(ctx context.Context, d *network.Device) (*ChangeSet, error)

	// Execute applies the changes to the device (requires prior Validate)
	Execute(ctx context.Context, d *network.Device) error

	// Name returns the operation name for logging
	Name() string
...
type Change struct {
	Table     string      `json:"table"`     // config_db table
	Key       string      `json:"key"`       // Entry key
...
func (cs *ChangeSet) AddChange(table, key, op string, oldVal, newVal interface{}) {
...
// applyPreview converts an operations.ChangeSet to a network.ChangeSet and
// applies it to the device via Redis. This is the bridge between the
// operations layer (which builds changes via Preview) and the network layer
// (which writes them to config_db).
func applyPreview(cs *ChangeSet, d *network.Device) error {
	ncs := network.NewChangeSet(cs.Device, cs.Operation)
	for _, c := range cs.Changes {
		var newValue map[string]string
		if c.NewValue != nil {
			var ok bool
			newValue, ok = c.NewValue.(map[string]string)
			if !ok {
				return fmt.Errorf("unexpected type for change value in %s|%s: %T", c.Table, c.Key, c.NewValue)
			}
		}
		ncs.Add(c.Table, c.Key, network.ChangeType(c.Operation), nil, newValue)
	}
	return ncs.Apply(d)
}
...
// MarkValidated marks the operation as validated
func (b *BaseOperation) MarkValidated() {
	b.validated = true
}

// IsValidated returns true if Validate() was called successfully
func (b *BaseOperation) IsValidated() bool {
	return b.validated
}

// RequireValidated returns error if not validated
func (b *BaseOperation) RequireValidated() error {
	if !b.validated {
		return fmt.Errorf("operation not validated: call Validate() first")
	}
	return nil
}
