...
func TestExecutorCountMatchesActionConstants(t *testing.T) {
	allActions := []StepAction{
		...
		ActionRunSuite, ActionSnapshot, ActionVerifySnapshot,
	}
...
// Scenario-path filename-prefix resolution moved entirely to
// pkg/newtrun/api/scenarios.go (the GET /scenario CRUD endpoint).
// Runner.Run matches --scenario against the scenario's `name:` field
// after LoadSuite has loaded the suite; there is no longer a path-
// resolution helper in pkg/newtrun. Tests for the old runner-side
// resolveScenarioPath were deleted with that function.
