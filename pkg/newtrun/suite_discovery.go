...
//
// Suites live at <networks-base>/<topology>/suites/<suite-name>/. The
...
// Globs <networks-base>/*/suites/<suite>/.
//
...
//     unambiguous.
func ResolveSuiteDir(networksBase, suite string) (string, error) {
	matches, err := filepath.Glob(filepath.Join(networksBase, "*", "suites", suite))
	if err != nil {
		return "", err
	}
	switch len(matches) {
...
// networksBase. Scans <networks-base>/*/suites/* and returns the
// sorted set of basenames. Missing networks-base directories return
// an empty slice rather than an error — a fresh checkout with no
// topologies tree is a valid deployment state.
func ListAllSuites(networksBase string) ([]string, error) {
	matches, err := filepath.Glob(filepath.Join(networksBase, "*", "suites", "*"))
	if err != nil {
		return nil, err
	}
