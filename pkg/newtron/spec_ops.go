...
func (net *Network) ListSpecInstances() ([]SpecInstance, error) {
...
func (net *Network) DeleteIPVPN(ctx context.Context, sel ScopeSelector, name string, opts ExecOpts) error {
	...
	if opts.Execute {
		if err := net.checkPermission(ctx, auth.PermSpecAuthor, auth.NewContext().WithField("ipvpns").WithResource(name)); err != nil {
			return err
		}
	}
	if !opts.Execute {
...
func (net *Network) DeleteMACVPN(ctx context.Context, sel ScopeSelector, name string, opts ExecOpts) error {
	...
	if opts.Execute {
		if err := net.checkPermission(ctx, auth.PermSpecAuthor, auth.NewContext().WithField("macvpns").WithResource(name)); err != nil {
			return err
		}
	}
	if !opts.Execute {
