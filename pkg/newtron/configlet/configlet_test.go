...
func TestMergeConfigDB_NilBase(t *testing.T) {
	overlay := map[string]map[string]map[string]string{
		"PORT": {
			"Ethernet0": {"speed": "100000"},
		},
	}

	result := MergeConfigDB(nil, overlay)
	if result["PORT"]["Ethernet0"]["speed"] != "100000" {
		t.Error("MergeConfigDB with nil base should return overlay data")
	}
}

func TestMergeConfigDB_FieldOverwrite(t *testing.T) {
	base := map[string]map[string]map[string]string{
		"PORT": {
			"Ethernet0": {"speed": "40000", "mtu": "9100"},
		},
	}
	overlay := map[string]map[string]map[string]string{
		"PORT": {
	...
	result := MergeConfigDB(base, overlay)
	if result["PORT"]["Ethernet0"]["speed"] != "100000" {
		t.Error("overlay should overwrite base field")
	}
	if result["PORT"]["Ethernet0"]["mtu"] != "9100" {
		t.Error("non-overlapping base fields should be preserved")
	}
}

func TestMergeConfigDB_NewTableAndKey(t *testing.T) {
	base := map[string]map[string]map[string]string{
		"PORT": {
			"Ethernet0": {"speed": "100000"},
		},
	...
	// New table added
	if result["VLAN"]["Vlan100"]["vlanid"] != "100" {
		t.Error("new table from overlay should be added")
	}

	// New key added to existing table
	if result["PORT"]["Ethernet4"]["speed"] != "25000" {
		t.Error("new key from overlay should be added")
	}

	// Original key preserved
	if result["PORT"]["Ethernet0"]["speed"] != "100000" {
		t.Error("original key in base should be preserved")
	}
}

func TestMergeConfigDB_EmptyOverlay(t *testing.T) {
	base := map[string]map[string]map[string]string{
		"PORT": {
			"Ethernet0": {"speed": "100000"},
		},
	...
	result := MergeConfigDB(base, map[string]map[string]map[string]string{})
	if result["PORT"]["Ethernet0"]["speed"] != "100000" {
		t.Error("empty overlay should leave base unchanged")
	}
}
