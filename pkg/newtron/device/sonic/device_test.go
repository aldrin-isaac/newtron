...
// TestProjection_CoversAllSchemaTables verifies that every table in the validation
// schema has a corresponding hydrator in configTableHydrators and a case in
// DeleteEntry. This prevents silent data loss in abstract/offline mode when
// new tables are added to the schema but not to the hydrator registry.
func TestProjection_CoversAllSchemaTables(t *testing.T) {
	db := NewConfigDB()

	for tableName := range Schema {
		testFields := map[string]string{"test_field": "test_value"}

		// Test hydrateConfigTable: should not panic and should create an entry
		db.hydrateConfigTable(tableName, "test_key", testFields)
...
// TestProjection_HydrateDeleteRoundTrip verifies that for each schema table,
// hydrateConfigTable creates state that DeleteEntry can remove.
func TestProjection_HydrateDeleteRoundTrip(t *testing.T) {
	for tableName := range Schema {
		t.Run(tableName, func(t *testing.T) {
			...
			db.hydrateConfigTable(tableName, "roundtrip_key", fields)
