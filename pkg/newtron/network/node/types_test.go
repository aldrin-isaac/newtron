...
func TestChangeSet_Add(t *testing.T) {
	...
	cs.Add("PORT", "Ethernet0", map[string]string{"mtu": "9100"})
...
func TestChangeSet_String_WithChanges(t *testing.T) {
	cs := NewChangeSet("test", "test")
	cs.Add("PORT", "Ethernet0", map[string]string{"mtu": "9100"})
	cs.Update("VLAN", "Vlan100", map[string]string{"vlanid": "100"})
	cs.Delete("VRF", "Vrf_CUST")
...
func TestInterface_HasService(t *testing.T) {
	t.Run("with service", func(t *testing.T) {
		...
		configDB.NewtronIntent["Ethernet0"] = map[string]string{"service_name": "CUSTOMER_L3", "state": "actuated", "operation": "apply-service", "name": "CUSTOMER_L3"}
...
func TestInterface_ServiceBindingProperties(t *testing.T) {
	...
	configDB.NewtronIntent["Ethernet0"] = map[string]string{
		"service_name": "CUSTOMER_L3",
		"ip_address":   "10.1.1.1/30",
		"vrf_name":     "CUSTOMER_L3_ETH0",
		"ipvpn":        "MGMT_SPOKE_GLOBAL",
		"macvpn":       "SERVER_VLAN",
		"ingress_acl":  "CUSTOMER_EDGE_IN",
		"egress_acl":   "CUSTOMER_EDGE_OUT",
		"state":        "actuated",
...
func TestInterface_String(t *testing.T) {
	...
	t.Run("interface with service", func(t *testing.T) {
		configDB := &sonic.ConfigDB{
			...
			PortChannelMember: map[string]map[string]string{},
			NewtronIntent: map[string]map[string]string{
				"Ethernet0": {"service_name": "CUSTOMER_L3", "state": "actuated", "operation": "apply-service", "name": "CUSTOMER_L3"},
			},
		}
		intf := stringTestIntf(
			sonic.PortEntry{AdminStatus: "up"},
...
// ============================================================================
// Node Property Tests (minimal, no connection required)
// ============================================================================

func TestNode_Name(t *testing.T) {
	n := &Node{name: "leaf1-ny"}
	if n.Name() != "leaf1-ny" {
		t.Errorf("Name() = %q, want %q", n.Name(), "leaf1-ny")
	}
}

func TestNode_IsConnected_NotConnected(t *testing.T) {
	n := &Node{connected: false}
	if n.IsConnected() {
		t.Error("IsConnected() should be false")
	}
}

func TestNode_IsLocked_NotLocked(t *testing.T) {
	n := &Node{locked: false}
	if n.IsLocked() {
