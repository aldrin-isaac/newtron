...
// TestResolveNodeSpecSecrets_PlaintextPassesThrough pins the §2.4
...
// TestResolveNodeSpecSecrets_ReferenceResolves pins the happy path
...
// TestResolveNodeSpecSecrets_MissingStoreFailsClosed pins that a
...
// TestResolveNodeSpecSecrets_Idempotent pins that running resolution
// twice on the same in-memory nodeSpec is safe — the loader caches
...
// TestResolveNodeSpecSecrets_NilNodeSpec pins the defensive no-op —
// callers should never hand us nil, but if they do we don't panic.
func TestResolveNodeSpecSecrets_NilNodeSpec(t *testing.T) {
	if err := resolveNodeSpecSecrets(nil, nil); err != nil {
		t.Errorf("nil nodeSpec: got error %v, want nil", err)
	}
}
