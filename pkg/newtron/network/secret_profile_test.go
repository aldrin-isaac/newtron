...
func TestResolveProfileSecrets_PlaintextPassesThrough(t *testing.T) {
	p := &spec.DeviceProfile{SSHUser: "admin", SSHPass: "literal-pass"}
	if err := resolveProfileSecrets(p, nil); err != nil {
...
func TestResolveProfileSecrets_ReferenceResolves(t *testing.T) {
	...
	p := &spec.DeviceProfile{SSHUser: "admin", SSHPass: "${secret:switch1_ssh_pass}"}
	if err := resolveProfileSecrets(p, store); err != nil {
		t.Fatalf("resolveProfileSecrets: %v", err)
	}
...
func TestResolveProfileSecrets_MissingStoreFailsClosed(t *testing.T) {
	p := &spec.DeviceProfile{SSHPass: "${secret:KEY}"}
	if err := resolveProfileSecrets(p, nil); err == nil {
		t.Error("resolveProfileSecrets succeeded on a reference with nil store; expected hard error")
	}
}
...
// pointers, so Network.loadProfile re-runs resolution on every
...
func TestResolveProfileSecrets_Idempotent(t *testing.T) {
	...
	p := &spec.DeviceProfile{SSHPass: "${secret:k}"}
	for i := 0; i < 3; i++ {
		if err := resolveProfileSecrets(p, store); err != nil {
...
func TestResolveProfileSecrets_NilProfile(t *testing.T) {
	if err := resolveProfileSecrets(nil, nil); err != nil {
