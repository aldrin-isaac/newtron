...
// ============================================================================
// generateServiceEntries Tests — L3 INTERFACE entries
// ============================================================================

func TestGenerateServiceEntries_L3_NoVRF(t *testing.T) {
	// Simulates fabric-underlay: L3 service with no VRF, no IPVPN.
	// Must produce a base INTERFACE entry (empty fields) + IP entry.
	// Without the base entry, SONiC intfmgrd won't process the IP.
	n := &Network{
		spec: &spec.NetworkSpecFile{
	...
	tp := &TopologyProvisioner{network: n}

	entries, err := tp.generateServiceEntries(
		"fabric-underlay",
		"Ethernet0",
		"10.1.0.0/31",
		nil,
		&spec.ResolvedProfile{},
	)
	if err != nil {
		t.Fatalf("generateServiceEntries() error: %v", err)
	}

	// Expect exactly 2 INTERFACE entries:
	//   1. INTERFACE|Ethernet0 (base, empty fields)
	//   2. INTERFACE|Ethernet0|10.1.0.0/31 (IP)
...
func TestGenerateServiceEntries_L3_WithVRF(t *testing.T) {
	// L3 service with interface VRF — base INTERFACE entry should have vrf_name.
	n := &Network{
		spec: &spec.NetworkSpecFile{
			Services: map[string]*spec.ServiceSpec{
				"customer-l3": {
